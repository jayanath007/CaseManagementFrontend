...
export function reducer(state = initialState, action: Actions.Any): State {
    ...
    switch (action.type) {
        case Actions.INIT_TIME_RECORDED_WIDGET:
            return { ...state, loading: false, fromToDate: { fromDate: dpsNewDate(action.timeOffset), toDate: dpsNewDate(action.timeOffset) } };
        ...
        case Actions.LOAD_DATA_SUCCESS:
            return { ...state, loading: false, data: action.payload.dataObj.data };
