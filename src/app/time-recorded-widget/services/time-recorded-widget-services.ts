...
@Injectable()
export class TimeRecordedWidgetService {
    ...
    public loadWorkDoneData(request: DataRequest) {
        return this.http.post<DataObjectResponse>(this.appConfig.serviceBase2 + '/Dashboard/GetTimeRecordedTimes?suppressErrors=true'
    ...
    public getFromToDate(periodId) {
        ...
        return this.http.get<FromToDateResponse>(this.appConfig.serviceBase2
            + '/Dashboard/GetFromToDateForTimeRecordedPeriod?suppressErrors=true&periodId=' + periodId).pipe(
                map((response) => response.data));
    }
