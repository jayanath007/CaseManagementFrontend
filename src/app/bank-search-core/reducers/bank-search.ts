...
export interface State {
    readonly views: { [token: string]: BankSearchState };
...
function getInitViewData(state: BankSearchState, action: Actions.InitPage): Partial<BankSearchState> {
    return {
        ...state,
        mainLoading: false,
...
export const getBankSearchColumnDefByToken = (token) => createSelector(getViewByToken(token),
    (view) => view ? view.columnDef : null);
export const getBankSearchPaginatorDefByToken = (token) => createSelector(getViewByToken(token),
    (view) => view ? view.paginatorDef : null);
export const getBankSearchRowsCountByToken = (token) => createSelector(getViewByToken(token),
